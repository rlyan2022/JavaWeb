...
<%
  ...
  <tr>
    ...
    <td align="center"><%if(!name.equals("admin")){ %><a href="#" onClick="window.open('manager.do?action=managerModifyQuery&id=<%=ID%>','','width=292,height=175')">权限设置</a><%}else{%>&nbsp;<%}%></td>
    <td align="center">
	<%if(!name.equals("admin")){ %><a href="manager.do?action=managerDel&id=<%=ID%>">删除</a><%}else{%>&nbsp;<%}%></td>
  </tr>
<%
  }
}
