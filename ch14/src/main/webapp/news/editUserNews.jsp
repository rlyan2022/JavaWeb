...
<%
	...
	out.print(newsT.getNewsId());
	*/
	String action=servlet.requestStr(request,"submit");
	if(action.equals("true")){
		if(newsT.getClassId() != 0){
			userN.upUserNews(newsT.getClassId(),newsT.getKindId(),newsT.getHeadTitle(),newsT.getContent(),newsT.getNewsFrom(),newsT.getNewsId());
  ...
  <tr>
    ...
    <td height="372" align="center" valign="top">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td height="22">&nbsp; <a href="personal.jsp">用户个人区</a>>><a href="userNews.jsp">添加文章</a></td>
        </tr>
      </table>
      <form action="editUserNews.jsp?submit=true" method="post" name="news" onSubmit = "return check();">
      <table width="98%" height="40%" border="1" align="center" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#CCCCCC">
          <tr align="center" background="../images/reg.gif">
            <td height="28" colspan="2"><font color="#FF0000" size="2"><strong>更新文章</strong></font></td>
          </tr>
   <%
	//列出显示的文章
	Iterator rs = userN.editUserNews(servlet.requestInt(request,"newsId"));
	while(rs.hasNext()){
	...
	%>
	  ...
	  %>
          ...
          <tr align="center">
            ...
            <td align="left">&nbsp; <input name="headTitle" type="text" class="text" id="headTitle" value="<%=tableNews.getHeadTitle()%>" size="30">
              &nbsp;&nbsp; <font color="#FF0000">必须填写</font></td>
          </tr>
          <tr align="center">
            <td height="245">文章内容</td>
            <td align="left">&nbsp; <textarea rows="15" name="content" cols="70" class="text"><%=tableNews.getContent()%></textarea>
            </td>
          </tr>
          <tr align="center">
            <td height="28">出处<br></td>
            <td align="left">&nbsp; <input name="newsFrom" type="text" class="text" id="newsFrom" value="<%=tableNews.getNewsFrom()%>" size="25"></td>
          </tr>
		  <%}%>
          <tr align="center">
            <td height="26" colspan="2"><table width="100%" height="26" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center"> <input type="hidden" name="newsId" value="<%=servlet.requestInt(request,"newsId")%>"><input name="submit" type="submit" value="更新">
