...
<html>
  <head>
	...
	function addItem() {
		window.self.location = "teacher.do?p=edit&pageNo=${teacherForm.pageNo}";
	}
	...
	function modifyItem() {
		...
		if (count == 1) {
			window.self.location = "teacher.do?p=edit&id=" +
			                        document.getElementsByName("selectFlag")[j].value +
			                        "&pageNo=${teacherForm.pageNo}";
		}
	}
	...
	function deleteItem() {
		...
		if (window.confirm("确认删除吗？")) {
			with (document.getElementById("teacherForm")) {
				method = "post";
				action = "teacher.do?p=delete&pageNo=${teacherForm.pageNo}";
				submit();
			}
		}
	}
	...
	function queryItem() {
		with (document.getElementById("teacherForm")) {
			method = "post";
			action = "teacher.do?p=list&pageNo=1";
			submit();
		}
	}
	...
	function topPage() {
		window.self.location = "teacher.do?p=list&pageNo=${pageModel.topPageNo}"
	}

	function previousPage() {
		window.self.location = "teacher.do?p=list&pageNo=${pageModel.previousPageNo}"
	}

	function nextPage() {
		window.self.location = "teacher.do?p=list&pageNo=${pageModel.nextPageNo}"
	}

	function bottomPage() {
		window.self.location = "teacher.do?p=list&pageNo=${pageModel.bottomPageNo}"
	}
  ...
  <body>
    ...
    <TABLE cellSpacing="1" cellPadding="2" width="60%" align="center" border="0">
      <TBODY>
      <TR>
        <TD width="522" class="p1" height="2" nowrap><img src="images/mark_arrow_02.gif" width="14" height="14">&nbsp;<b>教师管理</B></TD>
      </TR>
      </TBODY>
    </TABLE>
    ...
    <TABLE class=small cellSpacing="1" cellPadding="2" width="60%" align="center" bgColor="#000000" border="0">
      ...
      <TR>
          <TD class=TableData align=center>
            <font color="#FF0000">教师名:</font>&nbsp;&nbsp;<input type="text" name="name" id="name" class="BigInput" onkeypress="myOnkeypress()" size="40" maxlength="40" >
    ...
    <TABLE class=small cellSpacing="1" cellPadding="2" width="60%" align="center" bgColor="#000000" border="0">
      ...
      <TR>
      	  ...
      	  <TD class="TableSeparator" width="25%" align="center"><strong>教师名</strong></TD>
      ...
      <TR>
	      ...
	      <TD class=TableData align="center"><a href="teacher.do?p=detail&id=${item.id }">查看教师课程信息</a></td>
      </TR>
      </c:forEach>
